...
class AzmanPanelMainScreen(Screen):
    ...
    def prepare_menu(self):
        menu_definitions = [
            ...
            ("Picony", self.start_picon_install_flow, "icon_picons.png", "Pobieranie i zarządzanie piconami dla Twoich kanałów"),
    ...
    def start_picon_install_flow(self):
    ...
    def on_picon_path_selected(self, target_dir):
        ...
        self.progress_screen = self.session.open(DownloadProgressScreen, title="Pobieranie listy picon...")
        worker = PiconZipListWorker(
            picons_base_url=constants.PICONS_BASE_URL,
            callback_finished=self.on_picon_list_downloaded,
            parent_screen=self
        )
        worker.start()
    ...
    def on_picon_list_downloaded(self, parent_screen, error_message, picon_zip_filenames):
        ...
        self.open_selection_timer = eTimer()
        self.open_selection_timer.callback.append(boundFunction(self._open_multi_selection_screen, "paczki picon", item_list, self.on_picons_selected))
        self.open_selection_timer.start(1, True)
    ...
    def on_picons_selected(self, selected_zips):
        ...
        worker = PiconInstallationWorker(
            ...
            target_dir=self.picon_target_dir,
            base_url=constants.PICONS_BASE_URL,
            callback_progress=self._update_picon_progress_ui,
            callback_finished=self.on_picon_installation_finished
        )
        worker.start()

    def _update_picon_progress_ui(self, current_bytes, total_bytes, custom_title=None):
        if self.progress_screen:
    ...
    def on_picon_installation_finished(self, final_message):
        ...
        self.final_message_timer.callback.append(boundFunction(self._show_picon_summary_message, final_message))
        self.final_message_timer.start(1, True)

    def _show_picon_summary_message(self, final_message):
        self.session.open(MessageBox, f"Zakończono instalację picon.\n\n{final_message}", type=MessageBox.TYPE_INFO)
    ...
    def on_e2k_list_downloaded(self, error_message, found_items, temp_dir):
        ...
        self.open_selection_timer_e2k.callback.append(boundFunction(self.show_e2k_selection_screen, item_list))
...
class PiconPathSelectionScreen(Screen):
    skin = """
        <screen name="PiconPathSelectionScreen" position="center,center" size="1012,632" title="Wybierz lokalizację Picon" backgroundColor="#D9000000" flags="wfNoBorder">
            <ePixmap pixmap="skin_default/blank.png" position="0,0" size="1012,68" backgroundColor="black" zPosition="-1" />
            <widget source="title" render="Label" position="center,13" size="987,51" font="Regular;31" halign="center" foregroundColor="white" transparent="1"/>
            <widget name="list" position="13,76" size="987,460" scrollbarMode="showOnDemand" />
            <ePixmap pixmap="/usr/lib/enigma2/python/Plugins/Extensions/AzmanPanel/icons/red.png" position="25,557" size="37,51" alphatest="blend" />
            <widget source="key_red" render="Label" position="70,557" size="180,51" zPosition="3" font="Regular; 28" transparent="1" foregroundColor="white" />
            <ePixmap pixmap="/usr/lib/enigma2/python/Plugins/Extensions/AzmanPanel/icons/green.png" position="265,557" size="37,51" alphatest="blend" />
            <widget source="key_green" render="Label" position="310,557" size="290,51" zPosition="3" font="Regular; 28" transparent="1" foregroundColor="white" />
        </screen>
    """
    def __init__(self, session):
        Screen.__init__(self, session)
        ...
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions"], {
            ...
            "ok": self.select_path, "green": self.select_path,
        }, -1)
    ...
    def build_target_list(self):
        ...
        if not picon_dir_choices:
            parent_default_dir = os.path.dirname(constants.DEFAULT_PICON_TARGET_DIR)
            if os.path.exists(parent_default_dir) and os.access(parent_default_dir, os.W_OK):
                 picon_dir_choices.append((f"Domyślny ({constants.DEFAULT_PICON_TARGET_DIR})", constants.DEFAULT_PICON_TARGET_DIR))

        if not picon_dir_choices:
            self.session.openWithCallback(
                self.close_error, MessageBox, 
...
class DownloadProgressScreen(Screen):
    skin = """
        <screen position="center,center" size="800,150" title="Pobieranie...">
            <widget source="title" render="Label" position="10,10" size="780,50" font="Regular;24" halign="center" />
    ...
    def setProgress(self, current, total, custom_title=None):
        if custom_title:
            self["title"].setText(custom_title)
        ...
        if total > 0:
            ...
            self["progresstext"].setText("Proszę czekać...")