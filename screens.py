...
# ======================== OSTATECZNA POPRAWKA ========================
# Wczytujemy skin używając PEŁNEJ, ABSOLUTNEJ ścieżki do pliku.
# To omija wszystkie problematyczne mechanizmy wyszukiwania ścieżek w Enigmie.
loadSkin(f"{PLUGIN_PATH}/skin.xml")
# =====================================================================

class AzmanPanelMainScreen(Screen):
    # Nazwa ekranu w skin.xml musi być 'AzmanPanelMainScreen'
    ...
    def __init__(self, session):
        ...
        self["actions"] = ActionMap(["OkCancelActions", "DirectionActions"], {
            "cancel": self.close, "ok": self.run_selected_item,
    ...
    # ... reszta metod klasy AzmanPanelMainScreen pozostaje BEZ ZMIAN ...
    def prepare_menu(self):
        menu_definitions = [
            ("Dodatki E2K", self.open_e2k_addons_screen, "icon_e2k.png"),
    ...
    def run_selected_item(self):
        sel_r, sel_c = self.selected_pos
        item_index = self.current_page * self.items_per_page + (sel_r * self.GRID_COLS + sel_c)
        if item_index < len(self.items): self.items[item_index]['func']()
    def __onClose(self):
    ...
    def open_e2k_addons_screen(self): self.session.open(E2KAddonsScreen)
    def open_picon_management_screen(self): self.session.open(PiconManagementScreen)
    def download_and_install_sources_xml(self):
        if not os.path.exists(constants.SOURCES_XML_TARGET_DIR): os.makedirs(constants.SOURCES_XML_TARGET_DIR, exist_ok=True)
        self.session.openWithCallback(self._confirm_and_proceed_download_sources_xml, MessageBox, f"Czy na pewno chcesz pobrać i nadpisać plik: '{constants.SOURCES_XML_FILENAME}' w {constants.SOURCES_XML_TARGET_DIR}?", MessageBox.TYPE_YESNO, timeout=10, default=False)
    ...
    def _on_generic_download_finished(self, parent_screen, error_message, final_message, *args):
        ...
        self.final_epg_message_timer.callback.append(boundFunction(self.session.open, MessageBox, message, type=msg_type, timeout=10))
    ...
    def _find_bouquet_items(self, repo_path):
        ...
        for root, _, files in os.walk(repo_path):
            for file_name in files:
                ...
                if is_bouquet: found.append(os.path.join(root, file_name))
        return found
    def _list_download_finished(self, parent_screen, error_message, found_items, temp_extraction_dir):
        ...
        if error_message or not found_items:
            ...
            return
        self.temp_repo_dir, item_list = temp_extraction_dir, [(path, name) for name, path in found_items]
    ...
    def _on_multi_bouquet_selected(self, selected_paths):
        ...
        for path in selected_paths:
            try:
                ...
                if not os.path.exists(bouquets_tv_path) or dest_filename not in open(bouquets_tv_path, 'r', encoding='utf-8').read():
    ...
    def _iptv_org_pl_download_finished(self, parent_screen, error_message, final_message):
        ...
        self.final_bouquet_message_timer.callback.append(boundFunction(self.session.openWithCallback, self.open_bouquet_source_selection, MessageBox, message, type=msg_type, timeout=10))
        self.final_bouquet_message_timer.start(1, True)

# ... (reszta klas: AzmanSelectListScreen, E2KAddonsScreen, etc. pozostaje bez zmian) ...
# ... (Wklej tutaj pozostałe klasy, które nie były modyfikowane)
...
class AzmanSelectListScreen(Screen):
    ...
    def __init__(self, session, title, item_list):
        ...
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions", "DirectionActions"], {
            "ok": self.toggle_selection, "cancel": self.cancel, "red": self.cancel,
            "green": self.save, "yellow": self.toggle_all,
            "up": self["list"].up, "down": self["list"].down
        }, -1)
        self.onLayoutFinish.append(self.build_list)
    def build_list(self):
        self["list"].setList([(f"[{'x' if v in self.selected_items else ' '}] {dn}", v) for v, dn in self.item_list])
    def toggle_selection(self):
        current = self["list"].getCurrent()
        if not current: return
        v = current[1]
        if v in self.selected_items: self.selected_items.remove(v)
        else: self.selected_items.append(v)
        self.build_list()
    def toggle_all(self):
        all_v = [i[0] for i in self.item_list]
        if len(self.selected_items) == len(all_v): self.selected_items = []
        else: self.selected_items = all_v
        self.build_list()
    def save(self):
...
class E2KAddonsScreen(Screen):
    ...
    def __init__(self, session):
        ...
        self.session, self.progress_screen, self.changes_made, self.temp_dir, self.current_config = session, None, False, None, {}
        self["title"] = Label("E2K Dodatki")
        self["info_label"] = Label("Wybierz opcję, aby zarządzać dodatkami E2Kodi:\n   Zielony: Pluginy E2K\n   Żółty: Skiny E2K\n\nPobrana zostanie lista dostępnych dodatków,\nz której będzie można wybrać te do instalacji.")
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions"], {"cancel": self.ask_for_restart_on_exit, "green": self.start_plugin_download, "yellow": self.start_skin_download}, -1)
        self["key_green"], self["key_yellow"] = StaticText("Pluginy"), StaticText("Skiny")
    ...
    def ask_for_restart_on_exit(self):
        if self.changes_made:
            self.session.openWithCallback(self._prompt_gui_restart_callback_final, MessageBox, "Zainstalowano nowe dodatki E2K.\nZalecany jest restart GUI.\nCzy chcesz zrestartować GUI teraz?", MessageBox.TYPE_YESNO, timeout=15, default=True)
        else: self.close()
    ...
    def start_download(self, config):
        ...
        worker = E2KListDownloader(config=config, callback_progress=self._update_progress_ui, callback_finished=self.on_list_downloaded)
        worker.start()
    def start_skin_download(self):
        self.start_download({"type": "skinów", "url": constants.GITHUB_E2K_SKINS_REPO_URL, "repo_name": constants.E2K_SKINS_REPO_NAME, "target_dir": constants.E2K_SKINS_TARGET_DIR, "finder": self._find_skin_items})
    def start_plugin_download(self):
        self.start_download({"type": "pluginów", "url": constants.GITHUB_E2K_PLUGINS_ZIP_URL, "repo_name": constants.E2K_PLUGINS_REPO_NAME, "target_dir": constants.E2K_PLUGINS_TARGET_DIR, "finder": self._find_plugin_items})
    ...
    def show_selection_screen(self, item_list):
        self.session.openWithCallback(self.on_items_selected, AzmanSelectListScreen, f"Wybierz {self.current_config['type']} do instalacji", item_list)
    def on_items_selected(self, selected_paths):
        ...
        for source_path in selected_paths:
            try:
                ...
                print(f"[AzmanPanel E2K] Błąd instalacji {item_name}: {e}"); failed_count += 1
        self.cleanup()
        if installed_count > 0: self.changes_made = True
...
class BouquetSourceSelectionScreen(Screen):
    ...
    def __init__(self, session):
        ...
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions"], {
            "cancel": lambda: self.close(None), "red": lambda: self.close(None),
